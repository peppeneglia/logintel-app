...
export function ClientProfitability() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-white mb-3">Profittabilità Clienti</h1>

      {/* Summary cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-3">
        <div className="bg-gray-900 rounded-2xl border border-gray-800 p-4">
          <p className="text-xs text-gray-400">Ricavo totale clienti</p>
          <p className="text-xl font-bold text-white">€{totalRevenue.toLocaleString('it-IT')}</p>
        </div>
        <div className="bg-gray-900 rounded-2xl border border-gray-800 p-4">
          <p className="text-xs text-gray-400">Profitto totale</p>
          <p className="text-xl font-bold text-emerald-400">€{totalProfit.toLocaleString('it-IT')}</p>
        </div>
        <div className="bg-gray-900 rounded-2xl border border-gray-800 p-4">
          <p className="text-xs text-gray-400">Margine medio clienti</p>
      ...
      <div className="bg-gray-900 rounded-2xl border border-gray-800 p-6">
        <h2 className="text-sm font-semibold text-gray-300 mb-4">Profittabilità per cliente</h2>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-800">
                <th className="text-left py-3 px-3 font-medium text-gray-400">Cliente</th>
                <th className="text-right py-3 px-3 font-medium text-gray-400">Tratte</th>
                <th className="text-right py-3 px-3 font-medium text-gray-400">Ricavo totale</th>
                <th className="text-right py-3 px-3 font-medium text-gray-400">Costo totale</th>
                <th className="text-right py-3 px-3 font-medium text-gray-400">Profitto</th>
            ...
            <tbody>
              {mockClientProfitability.map((r) => (
                <tr key={r.id} className="border-b border-gray-800">
                  ...
                  <td className="py-3 px-3 text-gray-300 text-right">{r.tratte}</td>
                  <td className="py-3 px-3 text-gray-300 text-right">€{r.ricavoTotale.toLocaleString('it-IT')}</td>
                  <td className="py-3 px-3 text-gray-300 text-right">€{r.costoTotale.toLocaleString('it-IT')}</td>
