...
export function OperationalCosts() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-white mb-3">Costi Operativi</h1>

      {/* Summary Cards */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-3">
        <div className="bg-gray-900 rounded-2xl border border-gray-800 p-4">
          <p className="text-xs text-gray-400">Costo Totale Flotta</p>
          <p className="text-lg font-bold text-white">{euro(totals.totalCost)}</p>
        </div>
        <div className="bg-gray-900 rounded-2xl border border-gray-800 p-4">
          <p className="text-xs text-gray-400">Carburante</p>
          <p className="text-lg font-bold text-amber-400">{euro(totals.fuelCost)}</p>
        </div>
        <div className="bg-gray-900 rounded-2xl border border-gray-800 p-4">
          <p className="text-xs text-gray-400">Km Totali</p>
          <p className="text-lg font-bold text-primary-400">{totals.km.toLocaleString('it-IT')}</p>
        </div>
        <div className="bg-gray-900 rounded-2xl border border-gray-800 p-4">
          <p className="text-xs text-gray-400">Costo Medio /km</p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-800">
                <th className="text-left py-2 px-3 font-medium text-gray-400">Veicolo</th>
                <th className="text-right py-2 px-3 font-medium text-gray-400">Carburante</th>
                <th className="text-right py-2 px-3 font-medium text-gray-400">Manutenzione</th>
                <th className="text-right py-2 px-3 font-medium text-gray-400">Pedaggi</th>
                <th className="text-right py-2 px-3 font-medium text-gray-400">Assicurazione</th>
                <th className="text-right py-2 px-3 font-medium text-gray-400">Totale</th>
            ...
            <tbody>
              {mockOperationalCosts.map((cost) => (
                <tr key={cost.vehiclePlate} className="border-b border-gray-800">
                  <td className="py-2.5 px-3 font-medium text-white">{cost.vehiclePlate}</td>
                  <td className="py-2.5 px-3 text-right text-gray-300">{euro(cost.fuelCost)}</td>
                  <td className="py-2.5 px-3 text-right text-gray-300">{euro(cost.maintenanceCost)}</td>
                  <td className="py-2.5 px-3 text-right text-gray-300">{euro(cost.tollCost)}</td>
                  <td className="py-2.5 px-3 text-right text-gray-300">{euro(cost.insuranceCost)}</td>
                  <td className="py-2.5 px-3 text-right font-semibold text-white">{euro(cost.totalCost)}</td>
                  <td className="py-2.5 px-3 text-right text-gray-300">{cost.km.toLocaleString('it-IT')}</td>
                  <td className="py-2.5 px-3 text-right text-gray-400">{euro(cost.costPerKm)}</td>
                </tr>
              ))}
