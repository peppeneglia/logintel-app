...
export function VehicleAllocation() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-white mb-3">Allocazione Veicoli</h1>

      {/* Summary Row */}
      <div className="grid grid-cols-3 gap-4 mb-3">
        <div className="bg-gray-900 rounded-2xl border border-gray-800 p-4">
          <p className="text-xs text-gray-400">Programmati</p>
          <p className="text-lg font-bold text-primary-400">{scheduledCount}</p>
        </div>
        <div className="bg-gray-900 rounded-2xl border border-gray-800 p-4">
          <p className="text-xs text-gray-400">In Transito</p>
          <p className="text-lg font-bold text-amber-400">{inTransitCount}</p>
        </div>
        <div className="bg-gray-900 rounded-2xl border border-gray-800 p-4">
          <p className="text-xs text-gray-400">Completati</p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-800">
                ...
                <th className="text-left py-2 px-3 font-medium text-gray-400">Tratta</th>
                <th className="text-left py-2 px-3 font-medium text-gray-400">Autista</th>
                <th className="text-left py-2 px-3 font-medium text-gray-400">Partenza</th>
                <th className="text-left py-2 px-3 font-medium text-gray-400">Stato</th>
              </tr>
            </thead>
            <tbody>
              {mockVehicleAllocations.map((alloc) => (
                <tr key={alloc.id} className="border-b border-gray-800">
                  <td className="py-2.5 px-3 font-medium text-white">{alloc.vehiclePlate}</td>
                  <td className="py-2.5 px-3 text-gray-300">{alloc.route}</td>
                  <td className="py-2.5 px-3 text-gray-300">{alloc.driver}</td>
                  <td className="py-2.5 px-3 text-gray-300">{formatDateTime(alloc.departureDate)}</td>
