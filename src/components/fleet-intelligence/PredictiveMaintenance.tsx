...
export function PredictiveMaintenance() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-white mb-3">Manutenzione Predittiva</h1>

      {/* Summary Row */}
      <div className="grid grid-cols-3 gap-4 mb-3">
        <div className="bg-gray-900 rounded-2xl border border-gray-800 p-4">
          <p className="text-xs text-gray-400">Rischio Alto</p>
          <p className="text-lg font-bold text-red-400">{highCount}</p>
        </div>
        <div className="bg-gray-900 rounded-2xl border border-gray-800 p-4">
          <p className="text-xs text-gray-400">Rischio Medio</p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-800">
                ...
                <th className="text-left py-2 px-3 font-medium text-gray-400">Componente</th>
                <th className="text-left py-2 px-3 font-medium text-gray-400">Rischio</th>
                <th className="text-left py-2 px-3 font-medium text-gray-400">Data Prevista</th>
                <th className="text-right py-2 px-3 font-medium text-gray-400">Km al Guasto</th>
            ...
            <tbody>
              {mockMaintenanceAlerts.map((alert) => (
                <tr key={alert.id} className="border-b border-gray-800">
                  ...
                  <td className="py-2.5 px-3 text-gray-300">{alert.component}</td>
                  <td className="py-2.5 px-3">
                    <span className={`inline-block px-2.5 py-0.5 rounded-full text-xs font-medium ${riskBadge[alert.risk]}`}>
                      {riskLabel[alert.risk]}
                    </span>
                  </td>
                  <td className="py-2.5 px-3 text-gray-300">{formatDate(alert.predictedDate)}</td>
